...
public class ShoesActivity extends AppCompatActivity {
    ...
    //Initialisations
    private int SHOES_LIMIT_NBR = 50; //Maximum number of shoes we can create in the app
    ...
    ShoesFragment newShoesFragment;

    /**
     * @author Amélie Dieudonné
     * @param savedInstanceState Previous state of the activity
     * Called once when the activity is created (every time the ShoesActivity is open)
     */
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //if it's the first time the activity is created, create a new fragment and display it
        if (savedInstanceState == null) {
            newShoesFragment = new ShoesFragment();
            android.support.v4.app.FragmentTransaction ft = getSupportFragmentManager().beginTransaction();
            ft.add(R.id.RLfragment, newShoesFragment).commit();
        }


        //------------------------TOOLBAR------------------------------
        //Configure toolbar
        Toolbar toolbar = findViewById(R.id.toolbar_shoes);
        ...
        TextView TVnbrShoes = findViewById(R.id.TVnbrShoes);
        int nbrShoes = Shoes.getNbrShoes();
        TVnbrShoes.setText(""+nbrShoes);

        ShoesFragment.ActivityView = findViewById(android.R.id.content);
        ...
        btn_add.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (Shoes.getNbrShoes()< SHOES_LIMIT_NBR){
                    ...
                    Toast.makeText(ShoesActivity.this, R.string.too_many_shoes_msg, Toast.LENGTH_SHORT).show();
                }
    ...
    /**
     * @author Amélie Dieudonné
     * Open an alert dialog to add new shoes
     */
    public void openAlertDialog(){

        //Initialise the alert dialog
        final AlertDialog.Builder mBuilder = new AlertDialog.Builder(ShoesActivity.this);
        LayoutInflater inflater = (ShoesActivity.this).getLayoutInflater();
        View mView = inflater.inflate(R.layout.popup_activity_shoes, null);

        //Initialise the elements
        flipper = mView.findViewById(R.id.viewFlipper);

        ImageButton mShoes = mView.findViewById(R.id.shoes);
        ImageButton mBoots = mView.findViewById(R.id.boots);
        ImageButton mTong = mView.findViewById(R.id.tong);
        ImageButton mBasket = mView.findViewById(R.id.basket);
        ImageButton mHighheel = mView.findViewById(R.id.highheel);
        ImageButton mSandal = mView.findViewById(R.id.sandal);

        HashMap<ImageButton, DressingActivity.Type> hmp_shoes = new HashMap<>();
        hmp_shoes.put(mShoes, DressingActivity.Type.SHOES);
        hmp_shoes.put(mBoots, DressingActivity.Type.BOOTS);
        hmp_shoes.put(mTong, DressingActivity.Type.TONG);
        hmp_shoes.put(mBasket, DressingActivity.Type.BASKET);
        hmp_shoes.put(mHighheel, DressingActivity.Type.HIGHHEEL);
        hmp_shoes.put(mSandal, DressingActivity.Type.SANDAL);
        ...
        mOK.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (hasSelectedColor && kind_cloth != null && type_cloth != null) {
                    ...
                    Toast.makeText(ShoesActivity.this, R.string.success_add_msg, Toast.LENGTH_SHORT).show();

                    //add the cloth
                    new Shoes(type_cloth, color_cloth, kind_cloth);

                    //refresh the fragment
                    android.support.v4.app.FragmentTransaction ft = getSupportFragmentManager().beginTransaction();
                    ft.detach(newShoesFragment).attach(newShoesFragment).commit();
                    ...
                    Toast.makeText(ShoesActivity.this, R.string.error_add_msg, Toast.LENGTH_SHORT).show();
                }
            }
        });
    ...
    /**
     * @author Amélie Dieudonné
     * @param hmap Hash map linking every types buttons to their enumeration value
     * Get the type of the shoes the user want to create and stock it into the global variable type_cloth
     */
                ...
                public void onClick(View view) {
                    ...
                    for (ImageButton btn_img : hmap.keySet()){
                        ...
                        if (view==btn_img){
                            ...
                            btn_img.setBackgroundTintList(ContextCompat.getColorStateList(ShoesActivity.this, R.color.red));
                            for(ImageButton btn_NS : btn_not_selected.keySet()){
                                btn_NS.setBackgroundTintList(ContextCompat.getColorStateList(ShoesActivity.this, R.color.btnDefault));
                            }
                        }
                    }
                }
     ...
     * Get the color of the shoes the user want to create and stock it into the global variable color_cloth
     */
    ...
    /**
     * @author Amélie Dieudonné
     * @param list list of the 3 buttons specifying the kind of shoes (CASUAL, CHIC or SPORT)
     * Get the kind of the shoes the user want to create and stock it into the global variable kind_cloth
     */
    public void getKind(ArrayList<Button> list){
        //Getting the buttons back from the list
        final Button mCasual = list.get(0);
        final Button mChic = list.get(1);
        final Button mSport = list.get(2);

        //for every button, if it's clicked, the kind is this one
        for (final Button btn : list) {
            btn.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    //Initialise the 3 button as the default situation
                    Button selectedButton = mCasual;
                    Button notSelectedButton1 = mChic;
                    Button notSelectedButton2 = mSport;

                    //set the different button correctly
                    if (view == mCasual) {
                    ...
                    selectedButton.setBackgroundTintList(ContextCompat.getColorStateList(ShoesActivity.this, R.color.red));
                    selectedButton.setTextColor(ContextCompat.getColor(getApplicationContext(), R.color.white));
                    notSelectedButton1.setBackgroundTintList(ContextCompat.getColorStateList(ShoesActivity.this, R.color.btnDefault));
                    notSelectedButton1.setTextColor(ContextCompat.getColor(getApplicationContext(), R.color.black));
                    notSelectedButton2.setBackgroundTintList(ContextCompat.getColorStateList(ShoesActivity.this, R.color.btnDefault));
    ...
    /**
     * @author Amélie Dieudonné
     * @param view current view of the flipper
     * Go to the previous view of the flipper
     */
